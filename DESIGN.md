...
- `集計C++/ForWebpon.cpp`：スクレイピング結果→くじ単位クラスタリングのオフライン前処理。維持。
  将来、出力を `data/presets/*.json` へ変換するスクリプトを用意し、更新パイプラインを繋ぐ。
- 旧 `index.html` / `js/script.js` / `style.css`：エンジン検証が済むまで参照用に残す。
